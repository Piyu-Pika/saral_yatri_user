...
class ApiClient {
  ...
  static void init() {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      onResponse: (response, handler) {
        AppLogger.info('[API] ← ${response.statusCode} ${response.requestOptions.method} ${response.requestOptions.path}');
  ...
  static Future<Response> get(String path, {Map<String, dynamic>? queryParameters}) async {
    try {
      AppLogger.debug('[API] GET $path ${queryParameters != null ? 'with params: $queryParameters' : ''}');
  ...
  static Future<Response> post(String path, {dynamic data}) async {
    try {
      AppLogger.debug('[API] POST $path ${data != null ? 'with data' : 'without data'}');
  ...
  static Future<Response> put(String path, {dynamic data}) async {
    try {
      AppLogger.debug('[API] PUT $path ${data != null ? 'with data' : 'without data'}');
