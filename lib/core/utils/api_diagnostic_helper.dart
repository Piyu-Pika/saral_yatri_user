...
class ApiDiagnosticHelper {
  ...
  Future<Map<String, dynamic>> _testStationAPIs() async {
    ...
    try {
      ...
      final activeStationsResponse = await _apiService.get(ApiConstants.activeStations);
      ...
      if (stations.isNotEmpty) {
        ...
        try {
          final stationResponse = await _apiService.get('${ApiConstants.stationById}/$stationId');
          result['stationById'] = {
            'success': true,
            'stationId': stationId,
        ...
        // Test station search
        try {
          final searchResponse = await _apiService.get('${ApiConstants.searchStations}?q=bus');
  ...
  Future<Map<String, dynamic>> _testRouteAPIs() async {
    ...
    try {
      AppLogger.info('🛣️ Testing Route APIs (Critical for station-route issue)...');
      
      // Test active routes
      final activeRoutesResponse = await _apiService.get(ApiConstants.activeRoutes);
      ...
      if (routes.isNotEmpty) {
        ...
        try {
          ...
          final routeStationsResponse = await _apiService.get('${ApiConstants.routeActiveStations}/$routeId/stations/active');
          ...
          AppLogger.info('✅ Route stations: ${routeStations.length} stations found');
          
          // Analyze station-route relationships
          final stationsWithoutRoute = routeStations.where((s) => s['route_id'] != routeId).toList();
          if (stationsWithoutRoute.isNotEmpty) {
            result['routeStations']['warning'] = 'Some stations have mismatched route_id';
            AppLogger.warning('⚠️ Found ${stationsWithoutRoute.length} stations with mismatched route_id');
          }
          
        } catch (e) {
          result['routeStations'] = {'success': false, 'error': e.toString()};
          AppLogger.error('❌ Route stations failed: $e - THIS IS LIKELY THE STATION-ROUTE ISSUE!');
        }
        
        // Test route fare
        try {
          final routeFareResponse = await _apiService.get('${ApiConstants.routeFare}?route_id=$routeId');
          result['routeFare'] = {
            'success': true,
  ...
  Future<Map<String, dynamic>> _testBusAPIs() async {
    ...
    try {
      ...
      // Test active buses
      final activeBusesResponse = await _apiService.get(ApiConstants.activeBuses);
      ...
      if (buses.isNotEmpty) {
        ...
        // Test bus by number
        try {
          final busResponse = await _apiService.get('${ApiConstants.busById}?bus_number=$busNumber');
          result['busByNumber'] = {
            'success': true,
            'busNumber': busNumber,
        ...
        // Test buses by route
        try {
          final routeBusesResponse = await _apiService.get('${ApiConstants.busByRoute}?route_id=$routeId');
  ...
  Future<Map<String, dynamic>> _testFareCalculation() async {
    ...
    try {
      AppLogger.info('💰 Testing Fare Calculation (Critical for fare issue)...');
      
      // Get sample stations
      final stationsResponse = await _apiService.get(ApiConstants.activeStations);
      ...
      if (stations.length >= 2) {
        ...
        try {
          final fareResponse = await _apiService.post(ApiConstants.calculateFare, data: fareData);
          ...
          AppLogger.info('✅ Fare calculation successful: ₹${fareInfo['final_fare']}');
          ...
          AppLogger.error('❌ Fare calculation failed: $e - THIS IS THE FARE ISSUE!');
        }
      } else {
        result['fareCalculation'] = {
          'success': false,
  ...
  Future<Map<String, dynamic>> _testTicketOperations() async {
    ...
    try {
      AppLogger.info('🎫 Testing Ticket Operations (Critical for storage issue)...');
      ...
      try {
        ...
        if (tickets.isNotEmpty) {
          ...
          try {
            final ticketResponse = await _apiService.get('${ApiConstants.ticketDetails}/$ticketId');
        ...
        AppLogger.error('❌ My tickets failed: $e - THIS COULD BE THE STORAGE ISSUE!');
      }
      
      // Note: We don't test actual booking here to avoid creating test tickets
      result['bookingNote'] = 'Booking test skipped to avoid creating test data';
  ...
  Map<String, dynamic> _generateSummary(Map<String, dynamic> results) {
    ...
    if (results['routes']?['routeStations']?['success'] != true) {
      ...
      recommendations.add('Fix /routes/{id}/stations endpoint and verify station-route relationships');
    }
    ...
    if (results['fareCalculation']?['fareCalculation']?['success'] != true) {
      ...
      recommendations.add('Fix /tickets/calculate-fare endpoint and verify station distance data');
    }
    
    // Check for ticket storage issues
    if (results['tickets']?['myTickets']?['success'] != true) {
      issues.add('Ticket retrieval failing - Tickets not stored/retrieved properly');
      recommendations.add('Fix ticket booking flow and verify user-ticket associations in database');
    }
  ...
  Future<Map<String, dynamic>> attemptQuickFixes() async {
    ...
    try {
      ...
      int validRoutes = 0;
      for (final route in routes.take(3)) { // Test first 3 routes
        try {
          await _apiService.get('${ApiConstants.routeStations}/${route['id']}/stations');
          validRoutes++;
        } catch (e) {
      ...
      fixes['routeStationValidation'] = {'success': false, 'error': e.toString()};
    }
