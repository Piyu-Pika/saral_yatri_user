...
class TicketDataTest {
  ...
  static Future<void> testTicketDataResolution() async {
    try {
      ...
      final enhancedTickets =
          await repository.getUserTicketsWithResolvedNames();
      ...
      if (enhancedTickets.isNotEmpty) {
        ...
        Log.i(
            '   Destination Station: ${firstEnhanced.destinationStationName}');
  ...
  static Future<void> testDataResolutionAPIs() async {
    try {
      ...
      final stationName =
          await DataResolutionService.getStationName(sampleStationId);
