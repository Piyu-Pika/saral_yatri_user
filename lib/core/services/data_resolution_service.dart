...
class DataResolutionService {
  ...
  static Future<String> getStationName(String stationId) async {
    ...
    try {
      ...
      final response = await ApiClient.get('${ApiConstants.stationById}/$stationId');
      
      if (response.statusCode == 200) {
        final stationData = response.data['data'] ?? response.data['station'] ?? response.data;
        final stationName = stationData['name'] ?? stationData['station_name'] ?? 'Station $stationId';
  ...
  static Future<String> getBusNumber(String busId) async {
    ...
    try {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          final busData = response.data['data'] ?? response.data['bus'] ?? response.data;
          final busNumber = busData['bus_number'] ?? busData['number'] ?? busData['registration_number'] ?? 'Bus $busId';
        ...
        Log.w('Individual bus endpoint failed, trying active buses list: $individualError');
        
        // If individual endpoint fails (like 401), try to get from active buses list
        final activeBusesResponse = await ApiClient.get(ApiConstants.activeBuses);
        ...
        if (activeBusesResponse.statusCode == 200) {
          ...
          for (final bus in busList) {
            if (bus['id'] == busId) {
              final busNumber = bus['bus_number'] ?? bus['registration_number'] ?? 'Bus $busId';
              
              // Cache the result
              _busCache[busId] = busNumber;
              Log.i('Bus number resolved from active buses: $busId -> $busNumber');
              return busNumber;
            }
          }
      ...
      if (e.toString().contains('401') || e.toString().contains('unauthorized')) {
        Log.w('Authentication error when fetching bus data - user may need to log in');
      }
    }
  ...
  static Future<String> getRouteName(String routeId) async {
    ...
    try {
      ...
      final response = await ApiClient.get('${ApiConstants.routeById}/$routeId');
      
      if (response.statusCode == 200) {
        final routeData = response.data['data'] ?? response.data['route'] ?? response.data;
        final routeName = routeData['name'] ?? routeData['route_name'] ?? 'Route $routeId';
  ...
  static Future<Map<String, String>> resolveTicketData({
    ...
    try {
      Log.i('Resolving ticket data for stations: $boardingStationId, $destinationStationId, bus: $busId, route: $routeId');
  ...
  static Future<Map<String, String>> batchResolveStations(List<String> stationIds) async {
    final results = <String, String>{};
    
    // Filter out already cached stations
    final uncachedIds = stationIds.where((id) => !_stationCache.containsKey(id)).toList();
  ...
  static Future<Map<String, String>> batchResolveBusesFromActiveList(List<String> busIds) async {
    final results = <String, String>{};
    
    try {
      Log.i('Batch resolving ${busIds.length} bus numbers from active buses list');
      ...
      // Fallback to bus IDs
      for (final busId in busIds) {
        results[busId] = _isAuthError(e) 
            ? _handleAuthError('Bus', busId)
            : 'Bus $busId';
      }
    }
