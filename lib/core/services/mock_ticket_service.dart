...
class MockTicketService {
    ...
    return {
      "data": {
        ...
        "qr_data": {
          ...
          "fare_amount": 50,
          "subsidy_amount": 0,
          "government_signature": "GOVT-SIG-3638663736646538643931303434653765646434613935382d474f56543235313032313136353633322d3638663466346166333639663737343432626232343339342d323032342d30312d3135",
      ...
      "message": "Ticket booked successfully. Use QR data to generate QR code on frontend.",
      "instructions": {
        "qr_generation": "Use the qr_data object to generate QR code on frontend",
        "qr_libraries": ["qrcode.js", "react-qr-code", "vue-qrcode", "angular-qr"]
      },
      "success": true
    };
  }
  ...
  static EnhancedTicketModel createMockEnhancedTicket({
    ...
    if (boardingStationId != null) {
      ...
      mockResponse['data']['qr_data']['boarding_station_id'] = boardingStationId;
    }
    if (destinationStationId != null) {
      mockResponse['data']['ticket']['destination_station_id'] = destinationStationId;
      mockResponse['data']['qr_data']['destination_station_id'] = destinationStationId;
    }
    if (paymentMethod != null) {
      mockResponse['data']['ticket']['payment_details']['payment_mode'] = paymentMethod;
    }
    if (travelDate != null) {
      ...
      mockResponse['data']['ticket']['valid_until'] = validUntil.toIso8601String();
      mockResponse['data']['qr_data']['valid_until'] = validUntil.toIso8601String().split('T')[0];
    }
    ...
    mockResponse['data']['ticket']['payment_details']['payment_time'] = nowString;
    mockResponse['data']['ticket']['compliance_data']['revenue_date'] = nowString;
    mockResponse['data']['qr_data']['timestamp'] = nowString;
    
    // Generate new ticket number with current timestamp
    final ticketNumber = 'GOVT${now.millisecondsSinceEpoch.toString().substring(7)}';
    mockResponse['data']['ticket']['ticket_number'] = ticketNumber;
    mockResponse['data']['qr_data']['ticket_number'] = ticketNumber;
    
    // Update QR token with new data
