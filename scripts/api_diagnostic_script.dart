    ...
    try {
      ...
      if (activeStationsResponse.statusCode == 200) {
        ...
        if (stations.isNotEmpty) {
          ...
          print(
              '   Sample station: ${firstStation['name']} (ID: ${firstStation['id']})');
      ...
      final nearbyResponse =
          await dio.get('/stations/nearby?lat=28.6139&lng=77.2090');
    ...
    try {
      ...
      if (activeRoutesResponse.statusCode == 200) {
        ...
        if (routes.isNotEmpty) {
          ...
          print(
              '🔍 Testing route stations (Critical for active station issue)...');
          final routeStationsResponse =
              await dio.get('/routes/$routeId/stations');
          if (routeStationsResponse.statusCode == 200) {
            final routeStations = routeStationsResponse.data['data'] as List;
            print(
                '✅ Route stations: ${routeStations.length} stations found for route $routeId');

            // Analyze station data structure
            if (routeStations.isNotEmpty) {
              final sampleStation = routeStations.first;
              print(
                  '   Sample station structure: ${sampleStation.keys.toList()}');
              print(
                  '   Station: ${sampleStation['name']} (Sequence: ${sampleStation['sequence'] ?? 'N/A'})');
            }
          } else {
            print(
                '❌ Route stations endpoint failed - THIS COULD BE THE ISSUE!');
          }
        }
      } else {
    ...
    try {
      ...
      if (activeBusesResponse.statusCode == 200) {
        ...
        if (buses.isNotEmpty) {
          ...
          print(
              '   Sample bus: ${firstBus['bus_number']} (Route: ${firstBus['route_id']})');
          ...
          if (routeBusesResponse.statusCode == 200) {
            ...
            print(
                '✅ Buses by route: ${routeBuses.length} buses on route $routeId');
          }
        }
      } else {
    ...
    try {
      ...
      if (stationsResponse.statusCode == 200) {
        ...
        if (stations.length >= 2) {
          ...
          final fareResponse =
              await dio.post('/tickets/calculate-fare', data: fareData);
      ...
      if (stationsResponse.statusCode == 200 &&
        ...
        if (stations.length >= 2 && buses.isNotEmpty) {
          ...
          final bookingResponse =
              await dio.post('/tickets/passenger/book', data: bookingData);
    ...
    try {
      ...
      if (myTicketsResponse.statusCode == 200) {
        ...
        if (tickets.isNotEmpty) {
          ...
          print(
              '   Sample ticket: ${sampleTicket['id']} (Status: ${sampleTicket['status']})');
