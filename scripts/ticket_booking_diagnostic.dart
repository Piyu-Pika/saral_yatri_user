...
class TicketBookingStorageDiagnostic {
  ...
  Future<Map<String, dynamic>?> _getTestData() async {
    try {
      ...
      final stationsResponse =
          await dio.get('/routes/${testRoute['id']}/stations/active');
  ...
  Future<Map<String, dynamic>> _attemptBooking(
      Map<String, dynamic> testData) async {
    try {
      final stations = testData['stations'] as List;
  ...
  void _analyzeResults(
      int ticketsBefore, int ticketsAfter, Map<String, dynamic> bookingResult) {
    print('\n🔍 ANALYSIS RESULTS');
    print('=' * 40);
    ...
    if (bookingSuccess && ticketsAdded > 0) {
      ...
      print(
          '❌ CRITICAL ISSUE FOUND: Booking API succeeds but ticket not stored!');
