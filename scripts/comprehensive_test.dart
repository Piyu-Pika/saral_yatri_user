...
class ComprehensiveApiTester {
  ...
  Future<void> _testActiveStationsByRoute(String routeId) async {
    try {
      ...
      for (int i = 0; i < stations.length && i < 5; i++) {
        ...
        print('   ${i + 1}. ${stationData['station']['station_name']} - Distance: ${stationData['distance_from_start']} km');
      }
  ...
  Future<void> _testCalculateFare(Map<String, dynamic> testData) async {
    try {
      ...
      final stationsResponse = await dio.get('/routes/$routeId/stations/active');
  ...
  Future<void> _testGetMyTickets() async {
    try {
      ...
      for (int i = 0; i < tickets.length && i < 3; i++) {
        ...
        print('   ${i + 1}. ${ticket['ticket_number']} - ${ticket['status']} - ₹${ticket['fare_details']['final_amount']}');
      }
